...
Deno.serve(async (req: Request) => {
  ...
  const log = createLogger({ request_id: requestId, action: "webhooks-stripe" });

  try {
    const body = await req.text();
    const signature = req.headers.get("stripe-signature");

    log.info("Stripe webhook received", { has_signature: !!signature });
    ...
    if (event.type === "checkout.session.completed" || event.type === "payment_intent.succeeded") {
      ...
      if (!productId || !orgId || !userId) {
        log.warn("Missing metadata in webhook", { event_type: event.type });
      ...
      await supabase.from("events").insert({
        ...
        payload: { product_id: productId, provider: "stripe" },
